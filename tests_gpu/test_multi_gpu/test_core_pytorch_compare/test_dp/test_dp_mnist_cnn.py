...
class CNNNetAttributeTransfer(TTModel):
    def __init__(self):
        super(CNNNetAttributeTransfer, self).__init__()
        self.conv1 = nn.Conv2d(1, 32, 3, 1)
        self.conv2 = nn.Conv2d(32, 64, 3, 1)
        self.dropout1 = nn.Dropout2d(0.25)
        self.dropout2 = nn.Dropout2d(0.5)
        ...
        self.transfer_model_attributes = ['get_loss_ctr', 'get_predictions_ctr']
...
class TestMNISTCNNAttributeTransfer(unittest.TestCase):
    def test_trainloop_core_pytorch_compare(self):
        val_loss_tl, y_pred_tl, y_true_tl = self.train_eval_trainloop(num_epochs=5, use_real_train_data=True)
        val_loss_pt, y_pred_pt, y_true_pt = self.train_eval_core_pytorch(num_epochs=5, use_real_train_data=True)
    ...
    def train_eval_trainloop(self, num_epochs, use_real_train_data=False):
        ...
        self.assertEqual(tl.model.get_loss_ctr, (600 + 600 + 100) * num_epochs)

        val_loss = tl.evaluate_loss_on_validation_set(force_prediction=True)
        self.assertEqual(tl.model.get_loss_ctr, ((600 + 600 + 100) * num_epochs) + 100)

        y_pred, y_true, _ = tl.predict_on_validation_set(force_prediction=True)

        self.assertEqual(tl.model.get_predictions_ctr, 100)
...
"""
# class TestMNISTCNNExplicitDPDefinitionReverseCompatibility(unittest.TestCase):
#     def test_trainloop_core_pytorch_compare(self):
#         val_loss_tl, y_pred_tl, y_true_tl = self.train_eval_trainloop(num_epochs=5, use_real_train_data=True)
#         val_loss_pt, y_pred_pt, y_true_pt = self.train_eval_core_pytorch(num_epochs=5, use_real_train_data=True)
#
