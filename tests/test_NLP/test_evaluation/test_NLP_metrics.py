...
class TestROGUENonOfficialMetric(unittest.TestCase):
    def test_calculate_metric(self):
        rogue1 = ROUGENonPerlMetric('bla bla bla', 'bla bla bla')
        ...
        rogue2 = ROUGENonPerlMetric('Today the sun shines not', 'Today the sun does shine')
        ...
        rogue3 = ROUGENonPerlMetric('today we go to the university', 'yesterday he played basketball very long')
        self.assertEqual(rogue3.get_metric(),
                         {'rouge-1': {'f': 0.0, 'p': 0.0, 'r': 0.0}, 'rouge-2': {'f': 0.0, 'p': 0.0, 'r': 0.0},
                          'rouge-l': {'f': 0.0, 'p': 0.0, 'r': 0.0}})

        rogue4 = ROUGENonPerlMetric('today we go to the university', 'bla mjav how w!?')
