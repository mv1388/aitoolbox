...
class TestROGUENonOfficialMetric(unittest.TestCase):
    def test_calculate_metric(self):
        rogue1 = ROGUENonOfficialMetric('bla bla bla'.split(), 'bla bla bla'.split())
        ...
        rogue2 = ROGUENonOfficialMetric('Today the sun shines not'.split(), 'Today the sun does shine'.split())
        self.assertEqual(rogue2.get_metric(),
                         {'rouge-1': {'f': 0.5999999970000001, 'p': 0.6, 'r': 0.6},
                          'rouge-2': {'f': 0.0, 'p': 0.0, 'r': 0.0},
                          'rouge-l': {'f': 0.5999999999997, 'p': 0.6, 'r': 0.6}})

        rogue3 = ROGUENonOfficialMetric('today we go to the university'.split(),
